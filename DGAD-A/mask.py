...
def random_shrink_extend(flag, min_length=5, shrink_limit=1, extend_limit=2, pdb_id=None):
    #print(pdb_id, " random_shrink_extend")
    if pdb_id is None:
        print('Warning: pdb_id is None in random_shrink_extend, this might cause issues...')
        print('bonjour')
    first, last = continuous_flag_to_range(flag, pdb_id=pdb_id)
...
def continuous_flag_to_range(flag, pdb_id=None):
    #print(pdb_id, " continuous_flag_to_range")
...
@register_transform('mask_single_cdr_dgad')
class MaskSingleCDRDgad(object):
    ...
    def perform_masking_(self, data, selection=None, pdb_id=None):
        cdr_flag = data['cdr_flag']

        if selection is None:
            #Here definitly a cdr gets picked and thus the aug will work on it
            cdr_all = cdr_flag[cdr_flag > 0].unique().tolist()
            cdr_to_mask = random.choice(cdr_all)
        else:
            ...
            # Set preference
            if selection in cdr_all:
                cdr_to_mask = selection
            # First preference HCDR3, check if present in cdr_all
            elif constants.CDR.H3 in cdr_all:
                cdr_to_mask = constants.CDR.H3
            # Second preference LCDR3, check if present in cdr_all
            elif constants.CDR.L3 in cdr_all:
                #print(pdb_id, 'Fallback to L3')
                cdr_to_mask = constants.CDR.L3
            # Fallback to random cdr in cdr_all
            else:
                print(pdb_id,'No H3 or L3 found, picking random cdr')
                cdr_to_mask = random.choice(cdr_all)
        

        cdr_to_mask_flag = (cdr_flag == cdr_to_mask)
        if self.augmentation:
            cdr_to_mask_flag = random_shrink_extend(cdr_to_mask_flag, pdb_id=pdb_id)

        cdr_first, cdr_last = continuous_flag_to_range(cdr_to_mask_flag, pdb_id=pdb_id)
        left_idx = max(0, cdr_first-1)
        right_idx = min(data['aa'].size(0)-1, cdr_last+1)
        anchor_flag = torch.zeros(data['aa'].shape, dtype=torch.bool)
        anchor_flag[left_idx] = True
    ...
    def __call__(self, structure):
        #print(f"{structure['id']} Masking CDR {self.selection}.")
        if self.selection is None:
            ab_data = []
            if structure['heavy'] is not None:
                ab_data.append(structure['heavy'])
            ...
            data_to_mask = structure['heavy']
            if data_to_mask is None:
                #print(f"{structure['id']} Warning: No heavy chain data available for masking CDR3.")
                #print(f"{structure['id']} Fallback to light chain.")
                data_to_mask = structure['light']
            
            #if self.selection == constants.CDR.H3:
            #    print(f"{structure['id']} Masking H_CDR3")
            sel = int(self.selection)
        elif self.selection in (constants.CDR.L1, constants.CDR.L2, constants.CDR.L3, ):
            data_to_mask = structure['light']
            sel = int(self.selection)
        ...
        self.perform_masking_(data_to_mask, selection=sel, pdb_id=structure['id'])
        return structure
