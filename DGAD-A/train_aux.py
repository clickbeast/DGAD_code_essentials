...
from diffab.datasets.dgad_aux import AuxDataset
...
def setup_and_parse_args():
    ...
    parser.add_argument('--train_aux_from_ckpt', type=str, default=None)

    args = parser.parse_args()

    if args.train_aux_from_ckpt is not None:
        args.train_aux_from_ckpt = args.train_aux_from_ckpt.lower() == 'true'
...
def update_config_from_args(config, args):
    ...
    arg_to_config_map = {
        ...
        'train_aux_from_ckpt': 'dgad.sims.train_aux_from_ckpt',
    }
    ...
    for epoch in tracker.epochs(100):
        ...
        for batch in tracker(dataloader):  # Tracker wraps the dataloader!
            ...
            if not torch.isfinite(loss):
                ...
                ckpt_path = os.path.join(ckpt_dir, f'checkpoint_nan_{tracker.step}.pt')
...
def train_aux(dataloader,val_dataloader, sampler, tracker, model, optimizer, scheduler, args, config, run, ckpt_dir):
...
if __name__ == '__main__':
    ...
    if not os.path.exists(ckpt_dir):
        ...
        print(f'Logging checkpoints to {ckpt_dir}')
    ...
    aux_dataset = AuxDataset(dataset_dir=config.dgad.aux_dataset.aux_dataset_dir)

    aux_sampler = BalancedClusterSampler(aux_dataset, 
    ...
    collate.pad_chain_id = True
    collate.pad_icode = True 
    ...
    tracker = Tracker(aux_data_loader, aux_sampler, n_steps=config.train.max_iters)
    
    collate.tracker = tracker
    
    
    if config.dgad.sims.train_aux_from_ckpt:
        print('Setting tracker to 80K steps')
        tracker = Tracker(aux_data_loader, aux_sampler, n_steps=80000)
    ...
    print(f'Train {len(aux_dataset)} | Val {len(base_val_dataset)}')
    ...
    aux_model = get_model(config.model).to(args.device)

    aux_optimizer = get_optimizer(config.train.optimizer, aux_model)
    aux_scheduler = get_scheduler(config.train.scheduler, aux_optimizer)
    aux_optimizer.zero_grad()

    print('Number of parameters: %d' % count_parameters(aux_model))

    # Start from the base model for the aux model and fine tune this
    if config.dgad.sims.train_aux_from_ckpt:
        ckpt_path_aux = config.dgad.sims.aux_model_path
        #ckpt_path_base = config.dgad.sims.base_model_path
        print(f'Resuming from checkpoint for aux model: {ckpt_path_aux}')

        if not os.path.exists(ckpt_path_aux):
            raise FileNotFoundError(f'Checkpoint not found: {ckpt_path_aux}')

        ckpt = torch.load(ckpt_path_aux, map_location=args.device, weights_only=False)
        aux_model.load_state_dict(ckpt['model'])
        #print('Resuming optimizer states for aux model from base model...')
        #aux_optimizer.load_state_dict(ckpt['optimizer'])
        #print('Resuming scheduler states for aux model from base model...')
        #aux_scheduler.load_state_dict(ckpt['scheduler'])

        #print(f'Resuming iteration and epoch for aux model... {ckpt["iteration"]}, {ckpt["epoch"]}')
        #Fix resume here since basing from old training strategy just do 80k steps -> so no resume of tracker.
    ...
    ckpt_path = os.path.join(ckpt_dir, ckpt_name)
    tracker.save_checkpoint(aux_model, aux_optimizer, aux_scheduler, config, ckpt_path, complete_epoch=True)
