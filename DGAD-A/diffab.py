...
from diffab.modules.diffusion.dpm_full import FullDPM, FullDPMSims
...
@register_model('diffab_sims')
class DiffusionAntibodyDesignSims(nn.Module):

    def __init__(self, cfg, base_model=None, aux_model=None, guidance_strength=1.0, config = None, guidance_strength_per_component=False):
        super().__init__()
        self.cfg = cfg
        ...
        self.base_model: DiffusionAntibodyDesign = base_model
        self.aux_model: DiffusionAntibodyDesign = aux_model

        self.guidance_strength = guidance_strength


        self.diffusion = FullDPMSims(
            cfg.res_feat_dim,
            cfg.pair_feat_dim,
            **cfg.diffusion,
    ...
    def encode(self, batch, remove_structure, remove_sequence, model: DiffusionAntibodyDesign = None):
        """
        Returns:
            res_feat:   (N, L, res_feat_dim)
            pair_feat:  (N, L, L, pair_feat_dim)
    ...
    def sample_for_sims(
        ...
        base_mask_generate = base_batch['generate_flag']
        base_mask_res = base_batch['mask']
        base_res_feat, base_pair_feat, base_R_0, base_p_0 = self.encode(
            base_batch,
            remove_structure = sample_opt.get('sample_structure', True),
            remove_sequence = sample_opt.get('sample_sequence', True),
        ...
        base_v_0 = rotation_to_so3vec(base_R_0)
        base_s_0 = base_batch['aa']

        # Aux encoding

        aux_mask_generate = aux_batch['generate_flag']
        aux_mask_res = aux_batch['mask']
        aux_res_feat, aux_pair_feat, aux_R_0, aux_p_0 = self.encode(
            aux_batch,
            remove_structure = sample_opt.get('sample_structure', True),
            remove_sequence = sample_opt.get('sample_sequence', True),
        ...
        aux_v_0 = rotation_to_so3vec(aux_R_0)
        aux_s_0 = aux_batch['aa']



        traj = self.diffusion.sample_for_sims(
            base_v_0, base_p_0, base_s_0, base_res_feat, base_pair_feat, base_mask_generate, base_mask_res, 
            aux_v_0, aux_p_0, aux_s_0, aux_res_feat, aux_pair_feat, aux_mask_generate, aux_mask_res, 
            **sample_opt
            )
